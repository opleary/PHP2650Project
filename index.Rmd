...
In hyperparameter tuning, we found that an mtry value of 1 and a nodesize of 15 produced the lowest out-of-bag error (0.106).
...
lines(Time, pbc_pred$survival[1,])
...
plot(cif.predict3_pbc$`40`, col = "blue",
     ...
     xlab = "time",
     ylab = "Survival Probability")
lines(Time, pbc_pred$survival[2,])
...
plot(cif.predict3_pbc$`14`, col = "blue",
     ...
     xlab = "time",
     ylab = "Survival Probability")
lines(Time, pbc_pred$survival[3,])
...
| id  | time | event | CIF median survival | RSF median survival |
|-----|------|-------|---------------------|---------------------|
| 69  | 3395 | 1     | 1741                | 1413                |
| 40  | 1487 | 1     | 1536                | 1487                |
...
For the observations that experienced death, the conditional inference forest model predicts a median survival time closer to time of death than the random survival forest model does. For the censored individuals #140 and #153, both models predict the same median survival times for both observations.
...
We can compare the variable importance rankings between the random survival forest and the conditional inference forest. 
...
fit_comp_turn <- pec(list("cforest" = turn_cif, "rsfc" = turn_rf), data = turn_use, 

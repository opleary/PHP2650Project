...
The first dataset, the PBC dataset hails from the `survival` package in R and summarizes the survival data of primary biliary cirrhosis (PBC) patients from a randomized trial conducted between 1974 and 1984.
...
The prediction errors for each of our models will be implemented in the `pec` package in R. We set <img src="https://render.githubusercontent.com/render/math?math=B "/> = 50 and implement bootstrap cross-validation to get our estimates for the integrated brier scores. The models are assessed on the data samples that are NOT in the bootstrap sample (OOB data).
...
We'll start by splitting our data into a 75/25 training-testing set. Our training and testing sets have 313 and 105 observations respectively.
...
We can plot the predicted survival curves for 3 random individuals in our data and compare the predicted median survival time ( the time where the probability of survival = 0.5) to what is observed.
...
cif.predict3_pbc$`261`
cif.predict3_pbc$`186`
cif.predict3_pbc$`140`
...
plot(cif.predict3_pbc$`261`, col = "blue",
     main = "PBC Predicted Survival Curves for 3 Individuals",
     xlab = "time",
     ylab = "Survival Probability")
lines(cif.predict3_pbc$`186`, col = "green", add = T)
lines(cif.predict3_pbc$`140`, col = "orange", add = T)
...
| id  | time | event | median survival |
|-----|------|-------|-----------------|
| 261 | 1677 | 0     | 2288            |
| 186 | 1576 | 1     | 1576            |
| 140 | 3059 | 0     | 4191            |

The observation that experiences death dies at the exact time that the model predict that patient has a 50% chance of survival. The two other observations, which are censored, have more time beyond the scope of their observational periods before they have a 50% chance of surviving.
...
fit_comp_pbc <- pec(list("cforest" = pbc_cif),  
                    data = pbc_use,
...
set.seed(1)
rand_inds_turn <- sample(1:nrow(turn_use), 3)

cif.predict3_turn <- treeresponse(turn_cif$forest, newdata = turn_use[rand_inds_turn, ])
...
plot(cif.predict3_turn$`1`, col = "blue",
     main = "Turnover Predicted Survival Curves for 3 Individuals",
     xlab = "time",
     ylab = "Survival Probability")
lines(cif.predict3_turn$`2`, col = "green", add = T)
lines(cif.predict3_turn$`3`, col = "orange", add = T)
legend(x = "topright",          # Position
       legend = c("1", "2", "3"),  # Legend texts
       title = "id",           
       col = c("blue", "green", "orange"),          
       lwd = 2)    
...
We see that for the two individuals that experience turnover within the observational period, they turnover before their predicted median survival time. This is especially true for the second individual who only has a time of 1.41 before terminating their employment with their organization. Still, we see a trend in the median survival times, accurate to the order observed in time to event of the 3 individuals.
...
![CIF Variable Importance Employee Turnover Data. Industry, age, and traffic are among the top 3.](img/cif.var_imp_turn.png)

[Prediction Error Curves]{.underline}

We can compare the prediction error curves of the two models. Recall that these estimates are based on the average brier scores computed at different time points.

```{r, results='hide'}

fit_comp_turn <- pec(list("cforest" = turn_cif), data = turn_use, 
              formula=Surv(stag,event)~age + industry + profession + traffic + 
        greywage + way + selfcontrol + anxiety + age:way + way:selfcontrol,  
              splitMethod = "BootCv",
        B = 50,
        reference = FALSE
